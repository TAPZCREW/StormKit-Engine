...
export class TriangleApp: public App {
  ...
  private:
    auto update_framegraph(engine::FrameGraphBuilder&, std::atomic_bool&) -> void;
...
auto TriangleApp::run(std::span<const std::string_view> _) -> Int {
    m_application = engine::Application::create(APPLICATION_NAME, { 800u, 600u }, WINDOW_TITLE)
                      ...
                      .value();
    m_application->setUpdateFrameGraphCallback(bind_front(&TriangleApp::update_framegraph, this));
    ...
    m_vertex_shader = gpu::Shader::load_from_bytes(renderer.device(), SHADER_DATA, gpu::ShaderStageFlag::Vertex)
                        .transform_error(assert("Failed to load vertex shader"))
                        .value();

    m_fragment_shader = gpu::Shader::load_from_bytes(renderer.device(), SHADER_DATA, gpu::ShaderStageFlag::Fragment)
...
////////////////////////////////////////
////////////////////////////////////////
auto TriangleApp::update_framegraph(engine::FrameGraphBuilder& graph, std::atomic_bool& rebuild_graph) -> void {
    ...
    if (not initialized) {
        ...
        auto&& render_task = graph.add_raster_task<DrawTask>(
          "Render",
          [&](DrawTask& task_data, engine::GraphTaskBuilder& builder) {
              task_data.backbuffer = builder
                                       .create("color",
                                               engine::ImageDescription { .extent = m_window_extent,
                                                                          .type   = gpu::ImageType::T2D,
                                                                          .format = gpu::PixelFormat::BGRA8_UNORM })
                                       .id();

              graph.set_final_resource(task_data.backbuffer);
          },
          [this](const DrawTask& task_data, OptionalRef<gpu::RenderPass> render_pass, gpu::CommandBuffer& cmb) {
              if (not m_pipeline) {
                  const auto pileline_state = gpu::RasterPipelineState {
                      .viewport_state    = { .viewports = { { .extent = m_window_extent, .depth = { 0, 1 } } },
                                            .scissors  = { { .extent = m_window_extent } } },
                      .color_blend_state = { .attachments = { {} } },
                      .shader_state      = { .shaders = as_refs(m_vertex_shader, m_fragment_shader) },
                  };
                  auto& renderer = m_application->renderer();
                  m_pipeline     = gpu::Pipeline::create(renderer.device(), pipeline_state, m_pipeline_layout, *render_pass)
        ...
        render_task.set_cull_imune(true);
